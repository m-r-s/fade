function features = feature_extraction(signal, fs, listener)
% usage: features = feature_extraction(signal, fs, listener)
...
%
%
% Copyright (C) 2018 Marc René Schädler
...
if isempty(config) || ~isfield(config, configid)
  ...
  loss_spl_left = hl2spl(loss_freqs_left, loss_hl_left);
  loss_spl_right = hl2spl(loss_freqs_right, loss_hl_right);
...
signal = signal(1+round(fs.*0.100):end,:);
...
log_melspec_left = log_melspec_left + randn(size(log_melspec_left)).*0.1;
log_melspec_right = log_melspec_right + randn(size(log_melspec_right)).*0.1;
% Extract SGBFB features (with adapted parameters to compensate the spectral super-sampling)
features_left = sgbfb(log_melspec_left, [pi/4 pi/2]);
features_right = sgbfb(log_melspec_right, [pi/4 pi/2]);
